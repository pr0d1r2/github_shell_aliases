  ...
  do
    ...
    if [ $? -eq 0 ]; then
      curl \
        ...
        -i "`github_api_url_from_pull_request $github_pull_failed_build_urls_PARAM`/commits/`github_pull_last_hash $github_pull_failed_build_urls_PARAM`/status" 2>/dev/null | \
          grep target_url | \
          cut -f 4 -d '"'
    fi
  done
